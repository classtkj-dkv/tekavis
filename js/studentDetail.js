...
export function openStudentDetail(idPrefix, student, footerText = 'Class Tekavis') {
  ...
  const rows = [
    ['🏷️', 'Nama', student.name],
    ['🎂', hasBirthDate ? 'TTL' : 'Asal', hasBirthDate ? `${student.birth_place}, ${formatDate(student.birth_date)}` : student.birth_place],
  ...
  body.innerHTML = `
    <div class="id-card-photo" style="${student.photo_url ? `background-image:url('${student.photo_url}')` : ''}">
      ...
      <span class="id-card-major-badge">${student.major}</span>
